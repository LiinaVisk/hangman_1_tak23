...
class Model:
    ...
    def read_scores_data(self):
        # Loeb skoori andmed andmebaasist
        connection = None
        try:
    ...
    def new_game(self):
        # Alustab uut mängu, valides uue juhusliku sõna
        self.__wrong_count = 0
        self.__letters = []
        self.__correct_letters = []
        self.__wrong_letters = []
        # Uue juhusliku sõna valimine
        self.__word = self.random_word()
        self.__correct_letters = list("_" * len(self.__word))

    def random_word(self):
        # Loob juhusliku sõna andmebaasist
        connection = None
        try:
            connection = sqlite3.connect(self.__database)
    ...
    def check_user_input(self, text):
        # Kontrollib kasutaja sisestust ja uuendab vastavalt õigesti ja valesti arvatud tähti
        if text:
    ...
    def list_to_string(self, char_list):
        # Teisendab tähtede listi stringiks
