...
class View(Tk):
    def __init__(self, controller, model):
        ...
        # Loome "neli" siliti (label)
        self.__lbl_error, self.__lbl_time, self.__lbl_result = self.create_labels()
        # Loome sisestus kasti (teine näide eelnavlt õpitust )
    ...
    def create_frames(self):   # loob ja konfigureerib erinevad raamid aknas
    ...
    def create_buttons(self):  # Loob ja konfigureerib nupud
    ...
    def create_scoreboard_window(self):  #loob ja konfigureerib edetabeli akna
        top = Toplevel(self)
        top.title("Edetabel")
        top_w = 500
        top_h = 180
    ...
    def draw_scoreboard(self, frame, data):  #joonistab edetabeli
        if len(data) > 0:
            my_table = ttk.Treeview(frame)  # Tabeli vaade
    ...
    def change_image(self, image_id):  # vahetab pildi
